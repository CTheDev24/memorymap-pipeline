...
- Road widths, available highway types, and road height are configurable in `memorymap_pipeline/config.py` or via a JSON config passed with `--config`. Cycleways remain visible, while parking aisles, driveways, drive-throughs, emergency access, and private roads are omitted by default. Pedestrian paths and OSM `highway=track` agricultural/forestry access can be restored through `road_types` when wanted; athletic tracks use separate `leisure=track` tagging and are unaffected by this road filter. When terrain is enabled, motorways and trunks retain one elevation across each cross-section and use classification-specific smoothing only along travel. Their undersides remain embedded in the original relief; minor streets continue following terrain directly.
...
The Layer colors panel offers Urban Classic, Landscape Classic, Heritage, Gallery
Concrete, Nocturne, Ridgeline Sage, Desert Archive, Coastal Limestone, Deco After Dark,
Meridian Atlas, and Vector Lab collections. Each base, route, road, building, water, and
landscape swatch can also be changed independently; an individual edit switches the
collection to Custom and updates both the live preview and the material color embedded
in the generated 3MF.
